...
\centering
\begin{tikzpicture}[node distance=0.35cm and 1.1cm, scale=0.88, every node/.style={transform shape}]
  % Encoder stack
...
\centering
\begin{tikzpicture}[node distance=0.35cm, scale=0.92, every node/.style={transform shape}]
...
\centering
\begin{tikzpicture}[node distance=0.35cm, scale=0.94, every node/.style={transform shape}]
...
\centering
\begin{tikzpicture}[node distance=0.32cm, scale=0.92, every node/.style={transform shape}]
...
\centering
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{2.15cm} >{\raggedright\arraybackslash}p{2.1cm} >{\raggedright\arraybackslash}p{2.1cm} >{\raggedright\arraybackslash}p{2.3cm} >{\raggedright\arraybackslash}p{2.45cm}}
\toprule
Model & Core architecture & Main objective & Characteristic scale & Main conceptual shift \\
\midrule
Initial Transformer~\cite{vaswani2017attention} & Encoder--decoder & Seq2seq prediction & 6 encoder + 6 decoder layers in the paper & Attention replaces recurrence \\
\addlinespace[0.2em]
microGPT & Tiny decoder-only GPT & Character-level next-token prediction & 1 layer, 16 hidden, 4 heads & Educational minimal implementation \\
\addlinespace[0.2em]
GPT-1~\cite{radford2018gpt} & Decoder-only Transformer & Generative pretraining $+$ fine-tuning & 12 layers, 768 hidden & Transfer via generative pretraining \\
\addlinespace[0.2em]
GPT-2~\cite{radford2019gpt2} & Larger decoder-only Transformer & Large-scale next-token prediction & up to 1.5B params & Zero-shot behavior emerges \\
\addlinespace[0.2em]
GPT-3~\cite{brown2020gpt3} & Much larger decoder-only Transformer & Massive next-token prediction & 175B params & Few-shot / in-context learning \\
\bottomrule
\end{tabular}
\end{frame}
...
\text{decoder-only GPT family}
\quad\Longrightarrow\quad
\text{same basic skeleton, scaled aggressively.}
\]
\end{block}
\vspace{0.5em}
\begin{itemize}
  ...
  \item microGPT strips that decoder-only architecture down to its conceptual essentials.
  \item GPT-1, GPT-2, and GPT-3 mostly differ by \textbf{scale}, training data, and the behavioral consequences of scale.
\end{itemize}
\end{frame}
