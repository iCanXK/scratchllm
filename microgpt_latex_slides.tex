...
% Modern typography & font selection
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage[scaled=0.95]{helvet}
\renewcommand{\familydefault}{\sfdefault}
\usepackage{microtype}
\usepackage[varqu,varl]{inconsolata} % Crisp, modern code font
...
% Modernized Executive UT Dallas Color Palette
\definecolor{UTDGreen}{HTML}{053D2C}       % Primary Deep Forest Emerald
\definecolor{UTDOrange}{HTML}{E87500}      % Vibrant UT Dallas Amber
\definecolor{UTDSilverleaf}{HTML}{2A9D8F}  % Cyan/Teal accent
\definecolor{UTDDark}{HTML}{1E293B}        % Slate Dark text
\definecolor{UTDLightBg}{HTML}{F8FAFC}    % Clean canvas background
\definecolor{UTDCardBg}{HTML}{F1F5F9}     % Soft slate card
\definecolor{UTDGreenTint}{HTML}{EFF6FF}   % Soft Blue/Green tint
\definecolor{UTDOrangeTint}{HTML}{FFF7ED}  % Soft Amber tint
\definecolor{UTDGrayTint}{HTML}{F8FAFC}    % Soft Gray tint

% Backward-compatible color aliases
\colorlet{deepblue}{UTDGreen}
\colorlet{softblue}{UTDGreenTint}
\colorlet{softgreen}{UTDGreenTint}
\colorlet{softorange}{UTDOrangeTint}
\colorlet{softred}{UTDOrange!12}
\colorlet{softgray}{UTDCardBg}

% General Beamer Theme Overrides
\usetheme{default}
\useinnertheme{circles}
\setbeamertemplate{navigation symbols}{}
\setbeamercolor{background canvas}{bg=white}
\setbeamercolor{normal text}{fg=UTDDark}
\setbeamercolor{structure}{fg=UTDOrange}
\setbeamercolor{title}{fg=UTDGreen}
\setbeamercolor{frametitle}{fg=UTDGreen}
\setbeamercolor{section in toc}{fg=UTDGreen}
\setbeamercolor{subsection in toc}{fg=UTDDark}

% Custom Styled Card Blocks
\setbeamercolor{block title}{fg=white,bg=UTDGreen}
\setbeamercolor{block body}{fg=UTDDark,bg=UTDGreenTint}
\setbeamercolor{block title alerted}{fg=white,bg=UTDOrange}
\setbeamercolor{block body alerted}{fg=UTDDark,bg=UTDOrangeTint}
\setbeamercolor{block title example}{fg=white,bg=UTDSilverleaf}
\setbeamercolor{block body example}{fg=UTDDark,bg=UTDCardBg}

\setbeamertemplate{blocks}[rounded][shadow=false]

% Modern Minimalist Header (Frametitle + Category Tag)
\setbeamertemplate{frametitle}{%
  \nointerlineskip%
  \vspace*{0.3em}%
  \begin{beamercolorbox}[wd=\paperwidth]{frametitle}%
    \hspace*{1.2em}%
    \begin{minipage}{\dimexpr\paperwidth-2.4em\relax}%
      \ifx\insertsectionhead\empty\else
        {\usebeamerfont{headline}\fontseries{b}\selectfont\color{UTDOrange}\uppercase{\insertsectionhead}}\\[0.05em]%
      \fi
      {\usebeamerfont{frametitle}\fontseries{bx}\selectfont\color{UTDGreen}\insertframetitle}%
      \ifx\insertframesubtitle\empty\else
        \\[0.05em]{\usebeamerfont{framesubtitle}\color{UTDDark!70}\insertframesubtitle}%
      \fi
    \end{minipage}%
  \end{beamercolorbox}%
  \vspace*{0.2em}%
  \begin{beamercolorbox}[wd=\paperwidth,ht=0.8pt,dp=0pt]{block title alerted}%
  \end{beamercolorbox}%
}

% Modern Footer with Slide Counter and Accent Line
\setbeamertemplate{footline}{%
  \leavevmode%
  \begin{beamercolorbox}[wd=\paperwidth,ht=2.4ex,dp=1.0ex]{footline}%
    \hspace*{1.2em}%
    {\scriptsize\color{UTDDark!60}\insertshorttitle}%
    \hfill%
    {\scriptsize\color{UTDDark!60}\insertshortauthor}%
    \hfill%
    {\scriptsize\fontseries{b}\selectfont\color{UTDGreen}\insertframenumber\ \!\big/\ \!\inserttotalframenumber}%
    \hspace*{1.2em}%
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=\paperwidth,ht=1.2pt,dp=0pt]{block title alerted}%
  \end{beamercolorbox}%
}
...
\lstset{
  ...
  commentstyle=\color{UTDSilverleaf!85!black}\itshape,
  stringstyle=\color{UTDOrange!90!black},
  showstringspaces=false,
  columns=fullflexible,
  keepspaces=true,
  backgroundcolor=\color{UTDCardBg},
  frame=single,
  rulecolor=\color{UTDGreen!30},
  frameround=tttt,
  framesep=6pt,
  aboveskip=0.5em,
  belowskip=0.3em
...
% TikZ Modern Box & Arrow Definitions
\tikzset{
  box/.style={draw=UTDGreen, thick, rounded corners=5pt, align=center, fill=UTDGreenTint, minimum height=0.8cm, inner sep=6pt},
  greenbox/.style={draw=UTDGreen, thick, rounded corners=5pt, align=center, fill=UTDGreenTint, minimum height=0.8cm, inner sep=6pt},
  orangebox/.style={draw=UTDOrange, thick, rounded corners=5pt, align=center, fill=UTDOrangeTint, minimum height=0.8cm, inner sep=6pt},
  redbox/.style={draw=UTDOrange!85!black, thick, rounded corners=5pt, align=center, fill=UTDOrange!12, minimum height=0.8cm, inner sep=6pt},
  graybox/.style={draw=UTDDark!40, thick, rounded corners=5pt, align=center, fill=UTDCardBg, minimum height=0.8cm, inner sep=6pt},
  arr/.style={-{Stealth[length=2.5mm,width=2mm]}, thick, draw=UTDGreen},
  darr/.style={-{Stealth[length=2.5mm,width=2mm]}, thick, dashed, draw=UTDOrange}
}
...
% Sleek Modern Title Frame
\begin{frame}[plain]
\begin{tikzpicture}[remember picture,overlay]
  % Full canvas background
  \fill[UTDLightBg] (current page.south west) rectangle (current page.north east);
  
  % Left side geometric banner accent
  \fill[UTDGreen] (current page.south west) rectangle ([xshift=0.32\paperwidth]current page.north west);
  \fill[UTDOrange] ([xshift=0.32\paperwidth]current page.south west) rectangle ([xshift=0.332\paperwidth]current page.north west);

  % Title banner text on left
  \node[rotate=90, text=white, font=\bfseries\Huge] at ([xshift=0.14\paperwidth,yshift=0.50\paperheight]current page.south west) {microGPT};
  \node[rotate=90, text=UTDOrangeTint, font=\small\sffamily] at ([xshift=0.24\paperwidth,yshift=0.50\paperheight]current page.south west) {Understanding the inner workings of GPT models.};

  % Main Title & Metadata on right canvas
  \node[anchor=west, text=UTDGreen, align=left, font=\bfseries\fontsize{26}{30}\selectfont, text width=0.60\paperwidth] at ([xshift=0.37\paperwidth,yshift=0.22\paperheight]current page.north west) {How microGPT Works};
  
  \node[anchor=west, text=UTDOrange, font=\bfseries\large] at ([xshift=0.37\paperwidth,yshift=0.06\paperheight]current page.north west) {Number flow, layers, attention, \& training motivation};
  
  \draw[UTDGreen!25, line width=1.5pt] ([xshift=0.37\paperwidth,yshift=-0.02\paperheight]current page.north west) -- ([xshift=0.92\paperwidth,yshift=-0.02\paperheight]current page.north west);

  % Author / Badge Info
  \node[anchor=west, text=UTDDark, font=\large] at ([xshift=0.37\paperwidth,yshift=-0.12\paperheight]current page.north west) {\textbf{Author:} J Zhao};
  \node[anchor=west, text=UTDDark!75, font=\medium] at ([xshift=0.37\paperwidth,yshift=-0.19\paperheight]current page.north west) {\textbf{Topic:} Transformer Architecture Study Notes};
  
  % Date badge pill
  \node[anchor=west, draw=UTDGreen!40, fill=UTDGreenTint, rounded corners=4pt, inner sep=5pt, text=UTDGreen, font=\small\bfseries] at ([xshift=0.37\paperwidth,yshift=-0.28\paperheight]current page.north west) {\today};
\end{tikzpicture}
\end{frame}
...
\begin{frame}{MicroGPT architecture overview}
\centering
\includegraphics[width=\textwidth,height=0.80\textheight,keepaspectratio]{microgpt_decoder_architecture_diagram.png}

\vspace{0.15cm}
{\scriptsize Decoder-only Transformer: embeddings $\to$ causal self-attention blocks $\to$ output logits $\to$ next-token probabilities.}
\end{frame}
